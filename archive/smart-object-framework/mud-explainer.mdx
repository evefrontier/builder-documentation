...
- a resource type in the first 2 bytes ([RESOURCE_SYSTEM, RESOURCE_TABLE, or RESOURCE_NAMESPACE](https://github.com/latticexyz/mud/blob/main/packages/world/src/worldResourceTypes.sol))
...
The EVE Frontier World makes extensive use of the MUD System Libraries. Other deployment can also do so by adding the following to their `mud.config.ts` file:
...
This will create a library interface for the Systems defined in your `namespacess/my-namespace/systems/` folder. These Systems can now be interacted with as follows:

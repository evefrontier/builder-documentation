...
Much of this behaviour can be used as `modifiers` or `internal function calls` by inheriting from the `SmartObjectFramework` System.
All SOF integrated contracts should inherit from the ``SmartObjectFramework`` contract, since the SmartObjectFramework is where we extend the basic MUD framework with SOF quality of life features. 
The following are brief descriptions of the ``SmarObjectFramework``'s exposed functionality:

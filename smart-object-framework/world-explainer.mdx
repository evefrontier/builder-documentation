...
The Eve Frontier World is a deployed [MUD World](/smart/mud-explainer), and an accompanying suite of EVE Frontier game related System contract for managing on-chain behavior, state, and functionality of game item representative [Smart Objects](/smart/sof-explainer#classes-and-objects). 
...
Currently, Smart Character NFTs are __soulbound__ (tokens bound to a single account which cannot be transferred to others). However, in the future as Character transfers become active, this restriction will be lifted.
...
- when a Deployable is created, it also has an ERC721 NFT (with the same ID as the Deployable Object) minted to the player's account. Currently, similar to Smart Character, Deployable NFTs are [soulbound](/smart/world-explainer#smartcharactersol)
...
- ``UNNANCHORED`` or ``DESTROYED`` Deployables will have any [Inventory](/smart/world-explainer#inventorysol) (or [EphemeralInventory](/smart/world-explainer#ephemeralinventorysol)) items expunged from the chain and passed back to the game. to be handled by the game logic for each case individually. ``UNANCHORED`` SSU inventory items will be returned to the player's ship inventory. ``DESTORYED`` SSU invnetory items (and any EphemeralInventory items) are jettisoned as wrekage and subject to in-game destruction rules.
...
It should also be noted a Smart Deployable owner also has the permission to create [Hooks](/smart/sof-explainer#hooks-and-hook-inheritance) that target both ``bringOnline()`` and ``bringOffline()`` in case they want to trigger specific logic for these state changes on their Deployable.
...
- Both of the above are game actions and managed through the [ERC2771 MetaTxn](/smart/mud-explainer#meta-transactions-metatxns) flow
...
- Both of the above are considered game actions and are handled via the [MetaTxn](/smart/mud-explainer#meta-transactions-metatxns) flow
...
You can find a good behavioral description of usage for the above functionality in the [Smart Storage Unit](/SmartAssemblies/SmartStorageUnit#ephemeral-inventory-to-ssu-inventory-interaction) section of the documentation.

...
- a single entry point construction called the [World contract](https://github.com/latticexyz/mud/blob/next.17/packages/world/src/World.sol) which stores transaction data context and provides a single point of access to all of the registered resources of that world,
...
MUD employs these design differences to maximize composability and extension of an existing World as well as management of large smart contract codebase's, all of which lend themselves to the construction of complex [Autonomous Worlds](https://0xparc.org/blog/autonomous-worlds).

## Resources: Systems, Tables, and Namespaces

There are three primary resources in the MUD framework. Resources can be thought of as primitives or base components of the framework. Resources are assigned a [ResourceId](https://github.com/latticexyz/mud/blob/next.17/packages/store/src/ResourceId.sol) value which is used to uniquely identify each resource within the World.

The three define resources are:
- Systems
- Tables
- Namespaces
...
ResourceIds are a [deterministically generated](https://github.com/latticexyz/mud/blob/16121e8673adc72389a130cef64d73516c31d38c/packages/world/src/WorldResourceId.sol#L28) ``bytes32`` [user-defined value types](https://docs.soliditylang.org/en/v0.8.24/types.html#user-defined-value-types), being defined in three parts:
- a resource type in the first 2 bytes ([RESOURCE_SYSTEM, RESOURCE_TABLE, or RESOURCE_NAMESPACE](https://github.com/latticexyz/mud/blob/next.17/packages/world/src/worldResourceTypes.sol))
...
- the deployer of (and account who [registers](https://github.com/latticexyz/mud/blob/16121e8673adc72389a130cef64d73516c31d38c/packages/world/src/codegen/interfaces/IWorldRegistrationSystem.sol#L15)) a Namespace is the owner of that Namespace. 
- [Namespace owners](https://github.com/latticexyz/mud/blob/next.17/packages/world/src/codegen/tables/NamespaceOwner.sol) have rights to [register Systems](https://github.com/latticexyz/mud/blob/16121e8673adc72389a130cef64d73516c31d38c/packages/world/src/codegen/interfaces/IWorldRegistrationSystem.sol#L21) and Tables for that Namespace (tables are auto-registered to the Namespace defined in the ``mud.config.ts`` file)
- Namespace owners can [grant/revoke access](https://github.com/latticexyz/mud/blob/next.17/packages/world/src/modules/init/implementations/AccessManagementSystem.sol) to specific resources registered in a Namespace (access can be checked [here](https://github.com/latticexyz/mud/blob/next.17/packages/world/src/AccessControl.sol))
- Namespace owners can [transfer ownership](https://github.com/latticexyz/mud/blob/16121e8673adc72389a130cef64d73516c31d38c/packages/world/src/modules/init/implementations/AccessManagementSystem.sol#L55) to another account
...
- By default Table are publicly readable
...
The MUD framework provides three ways to interact with the World.

### Using world.call()

[world.call()](https://github.com/latticexyz/mud/blob/16121e8673adc72389a130cef64d73516c31d38c/packages/world/src/World.sol#L340) allows an external caller to make a call directly to a System within the MUD world, provided they know the ``ResourceId`` of the System they want to interact with, and the function selector of the method they want to call within that System. They can encode and pass the function selector together with the paramter values as they normally would for the [calldata](https://ethereum.org/en/developers/docs/transactions/#the-data-field) of a usual smart contract transaction.
...
- OtherSystem (the System being called) has [publicAccess=true](https://github.com/latticexyz/mud/blob/16121e8673adc72389a130cef64d73516c31d38c/packages/world/src/modules/init/implementations/WorldRegistrationSystem.sol#L121). ``publicAccess`` is configurable in the ``mud.config.ts``, or
...
In the case of the [EVE World](/smart/world-explainer#the-eve-world), we have set a delegation for the EVE World Systems and Tables Namespace, such that the entire Namespace allows for an [ERC2771 Trusted Forwarder](https://eips.ethereum.org/EIPS/eip-2771) contract to be the delagatee for any incoming call. In this way, the EVE World supports [ERC712 encoded](https://eips.ethereum.org/EIPS/eip-712) MetaTxns for game originating actions. 
...
- the MUD World is configured to only accept transactions from the Trusted Forwarder as a delagatee

...
- **Capability-Based Access:** Access control by action using a [capability](https://move-book.com/programmability/capability). Many operations require a  object (e.g., `OwnerCap`, `AdminCap`, `JumpPermit`) capability. The caller must own or hold the capability to perform the action. This pattern enables fine-grained, transferable permissions.
...
- **Hot Potato:** A one-time-use object that must be consumed within the same transaction. Used to enforce that a sequence of actions completes atomically (e.g., When a parent node goes offline, then all the connected assemblies shold go offline).
